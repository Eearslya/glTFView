...
void main() {
  ...
  outColor = In.Color * texColor;
		...
		const tk::SamplerCreateInfo samplerCI{.MagFilter        = vk::Filter::eLinear,
...
void ImGuiRenderer::Render(CommandBufferHandle& cmd, bool clear) {
	ImGui::EndFrame();
	...
	{
		RenderPassInfo rp = device.GetStockRenderPass(StockRenderPass::ColorOnly);
		if (!clear) {
