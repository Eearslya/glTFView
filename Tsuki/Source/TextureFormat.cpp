...
std::vector<vk::BufferImageCopy> TextureFormatLayout::BuildBufferImageCopies() const {
	...
	for (int level = 0; level < _mipLevels; ++level) {
		const auto& mip = _mips[level];

		copies[level] = vk::BufferImageCopy(mip.Offset,
		                                    mip.RowLength,
		                                    mip.ImageHeight,
		                                    vk::ImageSubresourceLayers(FormatToAspect(_format), level, 0, _arrayLayers),
		                                    vk::Offset3D(0, 0, 0),
		                                    vk::Extent3D(mip.Width, mip.Height, mip.Depth));
	}
...
void TextureFormatLayout::SetBuffer(void* buffer, size_t size) {
...
uint32_t TextureFormatLayout::MipLevels(uint32_t width, uint32_t height, uint32_t depth) {
...
void TextureFormatLayout::FillMipInfo(uint32_t width, uint32_t height, uint32_t depth) {
	...
	if (_mipLevels == 0) { _mipLevels = MipLevels(width, height, depth); }

	size_t offset = 0;
	for (uint32_t mip = 0; mip < _mipLevels; ++mip) {
		offset           = (offset + 15) & ~15;
		uint32_t blocksX = (width + _blockDimX - 1) / _blockDimX;
		uint32_t blocksY = (height + _blockDimY - 1) / _blockDimY;
		size_t mipSize   = blocksX * blocksY * _arrayLayers * depth * _blockStride;

		_mips[mip].Offset           = offset;
		_mips[mip].BlockRowLength   = blocksX;
		_mips[mip].BlockImageHeight = blocksY;
		_mips[mip].RowLength        = blocksX * _blockDimX;
		_mips[mip].ImageHeight      = blocksY * _blockDimY;
		_mips[mip].Width            = width;
		_mips[mip].Height           = height;
		_mips[mip].Depth            = depth;
