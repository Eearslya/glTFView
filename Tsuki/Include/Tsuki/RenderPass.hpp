...
#include <Tsuki/InternalSync.hpp>
...
class Framebuffer : public Cookie, public InternalSync {
 public:
...
template <>
struct tk::EnableBitmaskOperators<tk::DepthStencilOpBits> : std::true_type {};

template <>
struct std::hash<tk::RenderPassInfo> {
	size_t operator()(const tk::RenderPassInfo& info) {
		return static_cast<size_t>(tk::HashRenderPassInfo(info, false));
	}
};
